...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for(int i = 0; i < numReplicas; i++) {
			...
			replicafiles[i] = Hash.hashOf(filnavn);
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		//    		node.saveFileContent(filename, node.getNodeID(), bytesOfFile, primary);
		//    		counter++;
		//    	}
		...
		int randomReplica = random.nextInt(Util.numReplicas-1);
		...
		for(int i = 0; i < replicafiles.length; i++) {
			...
			if(i == randomReplica) {
				n.saveFileContent(filename, replica, bytesOfFile, true);
			}else {
				n.saveFileContent(filename, replica, bytesOfFile, false);
			}
	...
	public NodeInterface findPrimaryOfItem() throws RemoteException {
		...
		for(Message m: peers) {
			if(m.isPrimaryServer()) {
				BigInteger primary = m.getNodeID();
				NodeInterface successor = chordnode.findSuccessor(primary);
